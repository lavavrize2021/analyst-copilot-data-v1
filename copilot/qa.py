...
def answer_question(store,filing_id,question):
 ...
 if result:
  page=next((p for _,p in hits if p["page"]==result.get("page")),None); quote=result.get("quote","").strip(); normalized=lambda s:" ".join(s.lower().split()); valid=page and quote and normalized(quote) in normalized(page["text"])
